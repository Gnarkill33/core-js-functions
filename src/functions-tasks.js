...
function memoize(func) {
  ...
  return (...args) => {
    const funcKey = JSON.stringify(args);
    if (storage.has(funcKey)) {
      return storage.get(funcKey);
    }
    const result = func(...args);
    storage.set(funcKey, result);
    return result;
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    ...
    logFunc(`${functionName}(${functionArgs}) starts`);
    const result = func(...args);
    logFunc(`${functionName}(${functionArgs}) ends`);
    return result;
  };
}
...
function getIdGeneratorFunction(startFrom) {
  let invokingNum = startFrom;
  return () => {
    const currentNum = invokingNum;
    invokingNum += 1;
    return currentNum;
  };
}
